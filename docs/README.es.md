...
- Produce `agent_prompt` listo para usar como mensaje de sistema del LLM
...
| agent_prompt | Prompt personalizado listo para enviar al LLM |
...
1. **Usar `agent_prompt` directamente**: Enviar `agent_prompt` como mensaje de sistema o contexto previo; el LLM generará respuestas apropiadas basándose en los datos y acciones.
2. **Botones interactivos**: Convertir `suggested_actions` en botones de respuesta rápida para que el usuario seleccione.
3. **Ejecución automática**: Tras la confirmación del usuario, llamar automáticamente a la función correspondiente (ej. `create_expense` con sus `parameters`).
...
doc_vision.py  →  punto de entrada principal
   ...
   ├─ templates/         → plantillas Jinja2 para agent_prompt
